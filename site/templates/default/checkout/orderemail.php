...
<table width="850px" align="center" border="0" cellspacing="0" cellpadding="0" style="line-height:120%;">
  ...
  <tr class = "bg_gray">
     <td colspan = "2">
        <h3><?php print Text::_('JSHOP_EMAIL_PURCHASE_ORDER')?></h3>
     </td>
  </tr>
  <tr><td style="height:10px;font-size:1px;">&nbsp;</td></tr>
  <tr>
     <td width="50%">
        <?php print Text::_('JSHOP_ORDER_NUMBER')?>:
     </td>
     <td width="50%">
  ...
  <tr>
     <td>
        <?php print Text::_('JSHOP_ORDER_STATUS')?>:
     </td>
     <td>
...
<?php if ($this->show_customer_info){?>
  ...
  <tr class="bg_gray">
    <td colspan="2" width = "50%">
       <h3><?php print Text::_('JSHOP_CUSTOMER_INFORMATION')?></h3>
    </td>
  </tr>
  <tr>
    <td  style="vertical-align:top;padding-top:10px;" width = "50%">
      <table cellspacing="0" cellpadding="0" style="line-height:120%;">
        ...
        <tr>
          <td width="100"><?php print Text::_('JSHOP_REG_TITLE')?>:</td>
          <td><?php print $this->order->title?></td>
        </tr>
        <?php } ?>
        <?php if ($this->config_fields['firma_name']['display']){?>
        <tr>
          <td width="100"><?php print Text::_('JSHOP_FIRMA_NAME')?>:</td>
...
<?php print $this->_tmp_ext_html_ordermail_after_customer_info; ?>
  ...
  <tr>
    <td colspan = "2" class="bg_gray">
      <h3><?php print Text::_('JSHOP_ORDER_ITEMS')?></h3>
    </td>
  </tr>
  <tr>
    <td colspan="2" style="padding:0px;padding-top:10px;">
       <table width="100%" cellspacing="0" cellpadding="0" class="table_items">
        <tr><td colspan="5" style="vertical-align:top;padding-bottom:5px;font-size:1px;"><div style="height:1px;border-top:1px solid #999;"></div></td></tr>
         <tr class = "bold">            
            <td width="45%" style="padding-left:10px;padding-bottom:5px;"><?php print Text::_('JSHOP_NAME_PRODUCT')?></td>            
            <td width="15%" style="padding-bottom:5px;"><?php if ($this->config->show_product_code_in_order){?><?php print Text::_('JSHOP_EAN_PRODUCT')?><?php } ?></td>
            <td width="10%" style="padding-bottom:5px;"><?php print Text::_('JSHOP_QUANTITY')?></td>
            <td width="15%" style="padding-bottom:5px;"><?php print Text::_('JSHOP_SINGLEPRICE')?></td>
            <td width="15%" style="padding-bottom:5px;"><?php print Text::_('JSHOP_PRICE_TOTAL')?></td>
         </tr>
         ...
         <tr class="vertical">
           <td>
                ...
                <div class="manufacturer"><?php print Text::_('JSHOP_MANUFACTURER')?>: <span><?php print $prod->manufacturer?></span></div>
                <?php }?>
                ...
                <?php print Helper::sprintAtributeInOrder($prod->product_attributes)?>
                <?php print Helper::sprintFreeAtributeInOrder($prod->product_freeattributes)?>
                <?php print Helper::sprintExtraFiledsInOrder($prod->extra_fields)?>
                </div>
                <?php print $prod->_ext_attribute_html;?>
                <?php if ($this->config->display_delivery_time_for_product_in_order_mail && $prod->delivery_time){?>
                <div class="deliverytime"><?php print Text::_('JSHOP_DELIVERY_TIME')?>: <?php print $prod->delivery_time?></div>
                <?php }?>
           </td>           
           <td><?php if ($this->config->show_product_code_in_order){?><?php print $prod->product_ean;?><?php } ?></td>
           <td><span class="qtyval"><?php print Helper::formatqty($prod->product_quantity);?></span><?php print $prod->_qty_unit;?></td>
           <td>
                ...
                <?php if ($this->config->show_tax_product_in_cart && $prod->product_tax>0){?>
                    <div class="taxinfo"><?php print Helper::productTaxInfo($prod->product_tax, $order->display_price);?></div>
                <?php }?>
				<?php if ($this->config->cart_basic_price_show && $prod->basicprice>0){?>
                    <div class="basic_price"><?php print Text::_('JSHOP_BASIC_PRICE')?>: <span><?php print Helper::sprintBasicPrice($prod);?></span></div>
                <?php }?>
           </td>
           <td>
         ...
         <tr>
            ...
            <?php foreach($files as $file){?>
                <div><?php print $file->file_descr?> <a href="<?php print Uri::root()?>index.php?option=com_jshopping&controller=product&task=getfile&oid=<?php print $this->order->order_id?>&id=<?php print $file->id?>&hash=<?php print $this->order->file_hash?>&rl=1"><?php print Text::_('JSHOP_DOWNLOAD')?></a></div>
            <?php }?>    
            </td>
         </tr>
         <?php }?>
         ...
         <tr>
            <td colspan="5" style="text-align:right;font-size:12px;">
                <?php print Text::_('JSHOP_WEIGHT_PRODUCTS')?>: <span><?php print Helper::formatweight($this->order->weight);?></span>
            </td>
         </tr>   
         <?php }?>
      <?php if ($this->show_total_info){?>
         ...
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print Text::_('JSHOP_SUBTOTAL')?>:</td>
           <td class="price"><?php print Helper::formatprice($this->order->order_subtotal, $order->currency_code); ?><?php print $this->_tmp_ext_subtotal?></td>
         </tr>
         <?php } ?>
		 <?php print $this->_tmp_html_after_subtotal?>
         <?php if ($this->order->order_discount > 0){?>
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print Text::_('JSHOP_RABATT_VALUE')?><?php print $this->_tmp_ext_discount_text?>: </td>
           <td class="price">-<?php print Helper::formatprice($this->order->order_discount, $order->currency_code); ?><?php print $this->_tmp_ext_discount?></td>
         </tr>
         <?php } ?>
         <?php if (!$this->config->without_shipping){?>
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print Text::_('JSHOP_SHIPPING_PRICE')?>:</td>
           <td class="price"><?php print Helper::formatprice($this->order->order_shipping, $order->currency_code); ?><?php print $this->_tmp_ext_shipping?></td>
         </tr>
         <?php } ?>
         <?php if (!$this->config->without_shipping && ($order->order_package>0 || $this->config->display_null_package_price)){?>
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print Text::_('JSHOP_PACKAGE_PRICE')?>:</td>
           <td class="price"><?php print Helper::formatprice($this->order->order_package, $order->currency_code); ?><?php print $this->_tmp_ext_shipping_package?></td>
         </tr>
         <?php } ?>
         <?php if ($this->order->order_payment != 0){?>
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print $this->order->payment_name;?>:</td>
           <td class="price"><?php print Helper::formatprice($this->order->order_payment, $order->currency_code); ?><?php print $this->_tmp_ext_payment?></td>
         </tr>
         <?php } ?>
         <?php if (!$this->config->hide_tax){ ?>                           
         <?php foreach($this->order->order_tax_list as $percent=>$value){?>
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print Helper::displayTotalCartTaxName($order->display_price);?><?php if ($this->show_percent_tax) print " ".Helper::formattax($percent)."%";?>:</td>
             <td class="price"><?php print Helper::formatprice($value, $order->currency_code); ?><?php print $this->_tmp_ext_tax[$percent]?></td>
         </tr>
         <?php } ?>
         <?php } ?>
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><b><?php print $this->text_total ?>:</b></td>
           <td class="price"><b><?php print Helper::formatprice($this->order->order_total, $order->currency_code)?><?php print $this->_tmp_ext_total?></b></td>
         </tr>
         ...
         <?php if (!$this->client){?>
         <tr>
           <td colspan="5" class="bg_gray"><?php print Text::_('JSHOP_CUSTOMER_NOTE')?></td>
         </tr>
         <tr>
...
<?php if ($this->show_payment_shipping_info){?>
  ...
  <tr class = "bg_gray">
    ...
    <td>
        <h3><?php print Text::_('JSHOP_PAYMENT_INFORMATION')?></h3>
    </td>    
    <?php }?>
    <td <?php if ($this->config->without_payment){?> colspan="2" <?php }?>>
        <?php if (!$this->config->without_shipping){?>
        <h3><?php print Text::_('JSHOP_SHIPPING_INFORMATION')?></h3>
        <?php } ?>
    </td>    
  </tr>
    ...
    <td valign="top" <?php if ($this->config->without_payment){?> colspan="2" <?php }?>>
        <?php if (!$this->config->without_shipping){?>
            ...
            if ($this->config->show_delivery_date && $order->delivery_date_f){
                print "<div>".Text::_('JSHOP_DELIVERY_DATE').": ".$order->delivery_date_f."</div>";
            }
        }
        ?>
    </td>  
...
<?php }?>
  ...
  <tr>
    <td colspan="2"><br/><br/><a class = "policy" target="_blank" href="<?php print $this->liveurlhost.Helper::SEFLink('index.php?option=com_jshopping&controller=content&task=view&page=return_policy&tmpl=component&order_id='.$this->order->order_id, 1);?>"><?php print Text::_('JSHOP_RETURN_POLICY')?></a></td>
  </tr>
  <?php }?>
  <?php if ($this->client){?>
  <tr>
