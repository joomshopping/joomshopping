...
<form action="<?php print Helper::SEFLink('index.php?option=com_jshopping&controller=cart&task=refresh') ?>" method="post" name="updateCart" id="updateCart">
...
<?php if ($countprod > 0) : ?>
    ...
    <tr class="jshop_prod_cart <?php if ($i % 2 == 0) print "even"; else print "odd"?>">
        ...
        <td class="quantity">
            <div class="data">
				...
				<?php if (!$prod['not_qty_update']){?>
					<span class="cart_reload icon-refresh" title="<?php print Text::_('JSHOP_UPDATE_CART')?>"></span>
				<?php }?>
            </div>
        </td>
        ...
        <td class="remove">
            <div class="data">
                ...
                <?php } else { ?>
                    <a class="button-img btn btn-danger btn-sm" href="<?php print $prod['href_delete']?>" onclick="return confirm('<?php print Text::_('JSHOP_CONFIRM_REMOVE')?>')">
                        <?php print Text::_('JSHOP_REMOVE')?>
                    </a>
                <?php } ?>
            </div>
    ...
    <?php if ($this->config->show_cart_clear){?>
        <div class="clear-cart">
            <a class="btn btn-danger clear-cart" href="<?php print Helper::SEFLink('index.php?option=com_jshopping&controller=cart&task=clear')?>" onclick="return confirm('<?php print Text::_('JSHOP_CONFIRM_REMOVE_ALL')?>')">
    ...
    <?php if ($this->config->show_weight_order) : ?>
        <div class="weightorder">
            <?php print Text::_('JSHOP_WEIGHT_PRODUCTS')?>: <span><?php print Helper::formatweight($this->weight);?></span>
        </div>
    <?php endif; ?>
    ...
    <table class="jshop jshop_subtotal">
        ...
        <?php if ($this->free_discount > 0){?>
            <tr class="free_discount">
                <td colspan="2" align="right">
                    <span class="free_discount"><?php print Text::_('JSHOP_FREE_DISCOUNT')?>: <?php print Helper::formatprice($this->free_discount); ?></span>
                </td>
            </tr>
        <?php }?>
