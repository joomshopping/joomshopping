...
class ShippingMethodPriceTable extends ShopbaseTable{
	...
	function getPricesWeight($sh_pr_method_id, $id_country, &$cart){
        ...
        foreach($list as $k=>$v){
            ...
            $list[$k]->shipping_price = Helper::getPriceCalcParamsTax($list[$k]->shipping_price, $list[$k]->shipping_tax_id, $cart->products);
        }
        return $list; 
    }
    ...
    function calculateSum(&$cart){
        ...
        $prices['shipping'] = Helper::getPriceCalcParamsTax($prices['shipping'], $this->shipping_tax_id, $cart->products);
        $prices['package'] = floatval($prices['package']) * $JshopConfig->currency_value;
        $prices['package'] = Helper::getPriceCalcParamsTax($prices['package'], $this->package_tax_id, $cart->products);
    return $prices;
    }

    function calculateTax($sum){
        $JshopConfig = JSFactory::getConfig();
        $pricetax = Helper::getPriceTaxValue($sum, $this->getTax(), $JshopConfig->display_price_front_current);
        return $pricetax;
    }
    function calculateTaxPackage($sum){
        $JshopConfig = JSFactory::getConfig();
        $pricetax = Helper::getPriceTaxValue($sum, $this->getTaxPackage(), $JshopConfig->display_price_front_current);
        return $pricetax;
    }
