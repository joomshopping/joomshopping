...
<?php HelperAdmin::displaySubmenuConfigs('statictext');?>
  ...
  <tr class="row<?php echo $i % 2;?>">
   ...
   <td>
    <a href='index.php?option=com_jshopping&controller=config&task=statictextedit&id=<?php print $row->id?>'>    
		<?php if (Text::_('JSHP_STPAGE_'.$row->alias) != 'JSHP_STPAGE_'.$row->alias) print Text::_('JSHP_STPAGE_'.$row->alias); else print $row->alias;?>
    </a>
   </td>
   <td class="center">
     <?php echo HTMLHelper::_('jgrid.published', $row->use_for_return_policy, $i);?>
   </td>
   <td class="center">
   ...
   <?php if (!in_array($row->alias, $jshopConfig->sys_static_text)){?>
    <a class="btn btn-micro btn-nopad" href='index.php?option=com_jshopping&controller=config&task=deletestatictext&id=<?php print $row->id?>' onclick="return confirm('<?php print Text::_('JSHOP_DELETE')?>')">
