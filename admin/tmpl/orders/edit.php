...
jshopAdmin.lang_load='<?php print Text::_('JSHOP_LOAD')?>';
jshopAdmin.lang_price='<?php print Text::_('JSHOP_PRICE')?>';
jshopAdmin.lang_tax='<?php print Text::_('JSHOP_TAX')?>';
jshopAdmin.lang_weight='<?php print Text::_('JSHOP_PRODUCT_WEIGHT')?>';
jshopAdmin.lang_vendor='<?php print Text::_('JSHOP_VENDOR')?>';
jshopAdmin.cElName = '';
...
<?php if (!$order->order_created){?>
    <div class="row mb-2">
      <div class="col d-flex">
        <div class="mr-2"><b><?php print Text::_('JSHOP_FINISHED')?>:</b></div>
...
<div class="row mb-2">
    <div class="col d-flex">      
      <div class="mr-2 d-flex align-items-center"><b><?php print Text::_('JSHOP_USER')?>:</b></div>  
...
<?php if ($this->config->date_invoice_in_invoice){?>
    <div class="row mb-2">
      <div class="col d-flex"> 
          <div class="mr-2 d-flex align-items-center"><b><?php print Text::_('JSHOP_INVOICE_DATE')?>:</b></div>
          <div class="span10"><?php echo HTMLHelper::_('calendar', Helper::getDisplayDate($order->invoice_date, $this->config->store_date_format), 'invoice_date', 'invoice_date', $this->config->store_date_format , array('class'=>'inputbox', 'size'=>'25', 'maxlength'=>'19'));?></div>
      </div>
    </div>
<?php }?>
...
<tr>
    <td width="50%" valign="top">
        ...
        <tr id="tr_field_tax_number">
          <td><b><?php print Text::_('JSHOP_VAT_NUMBER')?>:</b></td>
        ...
        <tr>
          <td><b><?php print Text::_('JSHOP_BIRTHDAY')?>:</b></td>
          <td><?php echo HTMLHelper::_('calendar', $order->birthday, 'birthday', 'birthday', $this->config->field_birthday_format, array('class'=>'inputbox middle3', 'size'=>'25', 'maxlength'=>'19'));?></td>
        </tr>
        <?php } ?>
        <?php if ($config_fields['street']['display']){?>
        <tr>
          <td><b><?php print Text::_('JSHOP_STREET_NR')?>:</b></td>
          <td>
        ...
        <tr>
          <td><b><?php print Text::_('JSHOP_EXT_FIELD_1')?>:</b></td>
        ...
        <tr>
          <td><b><?php print Text::_('JSHOP_EXT_FIELD_2')?>:</b></td>
        ...
        <tr>
          <td><b><?php print Text::_('JSHOP_EXT_FIELD_3')?>:</b></td>
    ...
    <?php if ($this->count_filed_delivery >0) {?>
        ...
        <thead>
        <tr>
          <th colspan="2" align="center"><?php print Text::_('JSHOP_SHIP_TO')?></th>
        </tr>
        </thead>
        ...
        <tr>
          <td><b><?php print Text::_('JSHOP_BIRTHDAY')?>:</b></td>
          <td><?php echo HTMLHelper::_('calendar', $order->d_birthday, 'd_birthday', 'd_birthday', $this->config->field_birthday_format, array('class'=>'inputbox middle3', 'size'=>'25', 'maxlength'=>'19'));?></td>
        </tr>
        <?php } ?>
...
<div class="row">
    <div class="col-md-4"><?php print Text::_('JSHOP_CURRENCIES')?>: <?php print $this->select_currency?></div>
    <div class="col-md-4"><?php print Text::_('JSHOP_DISPLAY_PRICE')?>: <?php print $this->display_price_select?></div>
    <div class="col-md-4"><?php print Text::_('JSHOP_LANGUAGE_NAME')?>: <?php print $this->select_language?></div>
</div>
<br/>
...
<tr>
 ...
 <th>
   <?php echo Text::_('JSHOP_QUANTITY')?>
 </th> 
 <th width="16%">
...
<tr valign="top" id="order_item_row_<?php echo $i?>">
 <td>
   <input type="text" class = "form-control" name="product_name[<?php echo $i?>]" value="<?php echo $item->product_name?>" size="44" title="<?php print Text::_('JSHOP_TITLE')?>" />
   ...
   <input type="text" class = "form-control" name="manufacturer_code[<?php echo $i?>]" value="<?php echo $item->manufacturer_code?>" title="<?php print Text::_('JSHOP_MANUFACTURER_CODE')?>"><br />
   <?php }?>
   <?php if ($this->config->real_ean_in_cart){?>
   <input type="text" class="form-control" name="real_ean[<?php echo $i?>]" value="<?php echo $item->real_ean?>" title="<?php print Text::_('JSHOP_EAN')?>"><br />
   <?php }?>
   <?php if ($this->config->admin_show_attributes){?>
   <textarea rows="2" cols="24" name="product_attributes[<?php echo $i?>]" class = "form-control" title="<?php print Text::_('JSHOP_ATTRIBUTES')?>"><?php print $item->product_attributes?></textarea>   
   <br />
   <?php }?>
   <?php if ($this->config->admin_show_freeattributes){?>
   <textarea rows="2" cols="24" name="product_freeattributes[<?php echo $i?>]" class = "form-control" title="<?php print Text::_('JSHOP_FREE_ATTRIBUTES')?>"><?php print $item->product_freeattributes?></textarea>
   <?php }?>   
   ...
   <div>
   <?php echo Text::_('JSHOP_PRODUCT_WEIGHT')?> <input type="text" class = "form-control" name="weight[<?php echo $i?>]" value="<?php echo $item->weight?>" />
   </div>
   <div>   
   <?php echo Text::_('JSHOP_VENDOR')?> ID <input type="text" class = "form-control" name="vendor_id[<?php echo $i?>]" value="<?php echo $item->vendor_id?>" />
   </div>
   <?php }else{?>
 ...
 <td>
   <div class="price d-flex align-items-center"><div class="small-title"><?php print Text::_('JSHOP_PRICE')?>:</div> <input class="small3 form-control" type="text" name="product_item_price[<?php echo $i?>]" value="<?php echo $item->product_item_price;?>" onkeyup="jshopAdmin.updateOrderSubtotalValue();"/><?php echo ' '.$order->currency_code;?></div>
   <?php if (!$this->config->hide_tax){?>
   <div class="tax d-flex align-items-center"><div class="small-title"><?php print Text::_('JSHOP_TAX')?>:</div> <input class="small3 form-control " type="text" name="product_tax[<?php echo $i?>]" value="<?php echo $item->product_tax?>" /> %</div>
   <?php }?>
...
<div style="text-align:right;padding-top:3px;">
    <input type="button" class="btn btn-primary" value="<?php print Text::_('JSHOP_ADD')." ".Text::_('JSHOP_PRODUCT')?>" onclick="jshopAdmin.addOrderItemRow();">
</div>
...
<tr class="bold">
 <td class="right">
   <?php echo Text::_('JSHOP_COUPON_DISCOUNT')." (".Text::_('JSHOP_CODE').")"?>
 </td>
 <td class="left">
    <input type="text" class="small3 form-control" name="coupon_code" value="<?php echo $order->coupon_code?>">
	  <input type="button" class="btn btn-primary" value="<?php print Text::_('JSHOP_CALCULE')?>" onclick="jshopAdmin.order_discount_calculate();">
 </td>
</tr>
...
<tr class="bold">
 <td class="right">
     <?php print ($order->payment_name) ? $order->payment_name : Text::_('JSHOP_PAYMENT')?>
 </td>
 <td class="left">
...
<?php }?>
  <tr class="bold" id='row_button_add_tax'>
    <td></td>
    <td class="left">
    <input type="button" class="btn btn-primary" style="margin-top:5px" value="<?php print Text::_('JSHOP_TAX_CALCULATE')?>" onclick="jshopAdmin.order_tax_calculate();">
    <input type="button" class="btn btn-primary" style="margin-top:5px" value="<?php print Text::_('JSHOP_ADD')." ".Text::_('JSHOP_TAX')?>" onclick="jshopAdmin.addOrderTaxRow();">
    </td>
  </tr>
<?php }?>
...
<tr>
    <?php if (!$this->config->without_shipping){?>
    <th width="33%">
    <?php echo Text::_('JSHOP_SHIPPING_INFORMATION')?>
    </th>
    <?php }?>
    <?php if (!$this->config->without_payment){?>
    <th width="33%">
    <?php echo Text::_('JSHOP_PAYMENT_INFORMATION')?>
    </th>
    <?php } ?>
