...
<tr>
  <td width="50%" style="vertical-align:top;padding-top:14px;">
    ...
    <thead>    
      <tr>
        <th colspan="2">
          <?php echo Text::_('JSHOP_ORDER_PURCHASE')?>
        </th>
      </tr>
     </thead> 
      <tr>
        <td width="50%">
          <b><?php echo Text::_('JSHOP_NUMBER')?></b>
        </td>
        <td>
      ...
      <tr>
        <td width="50%">
          <b><?php echo Text::_('JSHOP_DATE')?></b> 
       </td>
        <td>
          <?php echo Helper::formatdate($order->order_date, 1);?>
        </td>
      </tr>
      <tr>
        <td>
          <b><?php echo Text::_('JSHOP_STATUS')?></b> 
       </td>
        <td>
      ...
      <tr>
        <td>
          <b><?php echo Text::_('JSHOP_IPADRESS')?></b>
       </td>
        <td>
  ...
  <td width="50%" style="vertical-align: top" class="input-inline">
      <ul class="joomla-tabs nav nav-tabs">
        <li class="nav-item"><a href="#first-page" class="nav-link active" data-toggle="tab"><?php echo Text::_('JSHOP_STATUS_CHANGE')?></a></li>
        <li><a href="#second-page" class="nav-link" data-toggle="tab"><?php echo Text::_('JSHOP_ORDER_HISTORY')?></a></li>
      </ul>
        ...
        <table>
          <tr>
            <th colspan="3" align="center">
                <?php echo Text::_('JSHOP_STATUS_CHANGE')?>:
            </th>
          </tr>
          <tr>
            <td>
              <?php echo Text::_('JSHOP_ORDER_STATUS')?>
            </td>
            <td>
              <?php echo HTMLHelper::_('select.genericlist', $lists['status'], 'order_status', 'class="inputbox form-select" style = "width: 250px"', 'status_id', 'name', $order->order_status ); ?>
            </td>
            <td style="padding-left:5px;">
              <input type="button" class="button btn btn-primary" name="update_status" onclick="jshopAdmin.verifyStatus(<?php echo $order->order_status?>, <?php echo $order->order_id?>, '<?php echo addslashes(Text::_('JSHOP_CHANGE_ORDER_STATUS'))?>', 1)" value="<?php echo addslashes(Text::_('JSHOP_UPDATE_STATUS'))?>" />
            </td>
          </tr>
          <tr>
              <td>
      ...
      <div id="second-page" class="tab-pane">
        <table class="table small">
            <tr class="bold">
              <td>
                <?php echo Text::_('JSHOP_DATE_ADDED')?>
              </td>
              <td>
			  ...
			  <td>
                <?php echo Text::_('JSHOP_INCLUDE_COMMENT')?>
              </td>
...
<tr>
 <td>
   ...
   <?php if ($this->config->manufacturer_code_in_cart && $item->manufacturer_code){?>
        <div class="manufacturer_code"><?php print Text::_('JSHOP_MANUFACTURER_CODE')?>: <span><?php print $item->manufacturer_code?></span></div>
   <?php }?>
   <?php if ($this->config->real_ean_in_cart && $item->real_ean){?>
        <div class="real_ean"><?php print Text::_('JSHOP_EAN')?>: <span><?php print $item->real_ean?></span></div>
   <?php }?>
   <?php print Helper::sprintAtributeInOrder($item->product_attributes).Helper::sprintFreeAtributeInOrder($item->product_freeattributes);?>
   <div class="extra_fields"><?php print Helper::sprintExtraFiledsInOrder($item->extra_fields)?></div>
   <?php print $item->_ext_attribute_html;?>
   <?php if (count($files)){?>
        <br/>
        <?php foreach($files as $file){?>
            <div><?php print $file->file_descr?> <a href="index.php?option=com_jshopping&controller=products&task=getfilesale&id=<?php print $file->id?>"><?php print Text::_('JSHOP_DOWNLOAD')?></a></div>
        <?php }?>
    <?php }?>
 ...
 <td>
   <?php if (isset($item->product_quantity)) echo Helper::formatqty($item->product_quantity)?><?php if (isset($item->_qty_unit)) print $item->_qty_unit?>
 </td> 
 <td>
...
<tr>
 <td colspan="5" style="height: 20px">
    ...
    <div style="text-align:right;">
        <i><?php print Text::_('JSHOP_WEIGHT_PRODUCTS')?>: <span><?php print Helper::formatweight($this->order->weight);?></span></i>
    </div><br/>
  <?php }?>
 </td>
</tr>
<tr class="bold">
 ...
 <td class="left" width="18%">
   <?php if (isset($order->order_subtotal) && isset($order->currency_code)) echo Helper::formatprice($order->order_subtotal, $order->currency_code);?><?php if (isset($this->_tmp_ext_subtotal)) print $this->_tmp_ext_subtotal?>
 </td>
</tr>
...
<tr class="bold">
 ...
 <td class="left">
   <?php if (isset($order->order_shipping) && isset($order->currency_code)) echo Helper::formatprice($order->order_shipping, $order->currency_code);?><?php if (isset($this->_tmp_ext_shipping)) print $this->_tmp_ext_shipping?>
 </td>
</tr>
<?php } ?>
...
<tr class = "bold">
 ...
 <td class = "left">
   <?php echo Helper::formatprice($order->order_package, $order->currency_code);?><?php print $this->_tmp_ext_shipping_package?>
 </td>
</tr>
<?php } ?>
...
<tr class="bold">
 ...
 <td class="left">
   <?php if (isset($order->order_payment) && isset($order->currency_code)) echo Helper::formatprice($order->order_payment, $order->currency_code);?><?php if (isset($this->_tmp_ext_payment)) print $this->_tmp_ext_payment?>
 </td>
</tr>
<?php } ?>
...
<?php if (!$this->config->hide_tax){?>
    <?php foreach($order->order_tax_list as $percent=>$value){?>
      <tr class="bold">
        ...
        <td  class="left">
          <?php if (isset($value) && isset($order->currency_code)) print Helper::formatprice($value, $order->currency_code);?><?php if (isset($this->_tmp_ext_tax[$percent])) print $this->_tmp_ext_tax[$percent]?>
        </td>
      </tr>
    <?php }?>
<?php }?>
<tr class="bold">
 ...
 <td class="left">
   <?php if (isset($order->order_total) && isset($order->currency_code)) echo Helper::formatprice($order->order_total, $order->currency_code);?><?php if (isset($this->_tmp_ext_total)) print $this->_tmp_ext_total?>
 </td>
</tr>
...
<tr>
    ...
    <?php } ?>
    <th width="34%">
    <?php echo Text::_('JSHOP_CUSTOMER_COMMENT')?>
    </th>
</tr>
</thead>
<tr>
...
<tr>
    <th width="50%">
        <?php echo Text::_('JSHOP_FILE_SALE')?>
    </th>
    <th>
        <?php echo Text::_('JSHOP_COUNT_DOWNLOAD')?>
    </th>
    <th>
...
<tr>
    ...
    <td>
        <?php if ($v->time) print Helper::formatdate($v->time, 1)?>
    </td>
</tr>
<?php }?>
</table>
<div class="order_stat_file_download_clear">
    <a onclick="return confirm('<?php print Text::_('JSHOP_CLEAR')?>')" href="index.php?option=com_jshopping&controller=orders&task=stat_file_download_clear&order_id=<?php print $order->order_id?>"><?php print Text::_('JSHOP_CLEAR')?></a>
</div>
<?php }?>
