...
<?php print $this->tmp_html_start?>
    <ul class="joomla-tabs nav nav-tabs">
        ...
        <?php }?>
        <li class="nav-item"><a href="#main-page" class="nav-link" data-toggle="tab"><?php echo JText::_('JSHOP_MAIN_PARAMETERS')?></a></li>
        <li class="nav-item"><a href="#image" class="nav-link" data-toggle="tab"><?php echo JText::_('JSHOP_IMAGE')?></a></li>
    </ul>
   ...
   <div id="main-page" class="tab-pane">
     ...
     <td class="key">
       ...
       <tr>
         ...
         <td>
           <input type="text" class="inputbox form-control" id="products_row" name="products_row" value="<?php echo $count_product_row=($row->manufacturer_id) ? ($row->products_row) : ($jshopConfig->count_products_to_row);?>" />
         </td>
       </tr>
     ...
     <div class="jshop_quote" id="image_manufacturer">
        <div>
            ...
            <div class="link_delete_foto">
                <a class="btn btn-micro" href="#" onclick="if (confirm('<?php print JText::_('JSHOP_DELETE_IMAGE')?>')) jshopAdmin.deleteFotoManufacturer('<?php echo $row->manufacturer_id?>');return false;">
   ...
   <script type="text/javascript">   
     Joomla.submitbutton=function(task){
        ...
        if (task == 'save' || task == 'apply'){
            if (!(jQuery('#products_page').val())){
               alert ('<?php echo JText::_('JSHOP_WRITE_PRODUCTS_PAGE')?>');
               return 0;
             } else if (!(jQuery('#products_row')).val()){
               alert ('<?php echo JText::_('JSHOP_WRITE_PRODUCTS_ROW')?>');
               return 0;
            } else if (!jQuery('#category_width_image').val() && !jQuery('#category_height_image').val()){
