...
class ShippingsModel extends BaseadminModel{
    ...
    function savePrices($sh_pr_method_id, $array_post) {
        ...
        foreach($array_post['shipping_price'] as $key => $value){
            ...
            $sh_method->shipping_price = Helper::saveAsPrice($array_post['shipping_price'][$key]);
            $sh_method->shipping_package_price = Helper::saveAsPrice($array_post['shipping_package_price'][$key]);
            $sh_method->shipping_weight_from = Helper::saveAsPrice($array_post['shipping_weight_from'][$key]);
            $sh_method->shipping_weight_to = Helper::saveAsPrice($array_post['shipping_weight_to'][$key]);
            if (!$sh_method->store()) {
    ...
    public function save(array $post){
		...
		if (!$shipping->store()){
			$this->setError(Text::_('JSHOP_ERROR_SAVE_DATABASE')." ".$shipping->getError());
			return 0;
		}
